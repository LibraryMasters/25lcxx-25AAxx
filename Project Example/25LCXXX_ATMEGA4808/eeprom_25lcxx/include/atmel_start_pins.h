 ...
 * \param[in] pull_mode Pin pull mode
 */
static inline void SPI_NSS_set_pull_mode(const enum port_pull_mode pull_mode)
{
	PORTC_set_pin_pull_mode(3, pull_mode);
}

/**
 * \brief Set SPI_NSS data direction
 *
 * Select if the pin data direction is input, output or disabled.
 * If disabled state is not possible, this function throws an assert.
 *
 ...
 *                      (low power state)
 */
static inline void SPI_NSS_set_dir(const enum port_dir dir)
{
	PORTC_set_pin_dir(3, dir);
}

/**
 * \brief Set SPI_NSS input/sense configuration
 *
 * Enable/disable SPI_NSS digital input buffer and pin change interrupt,
 * select pin interrupt edge/level sensing mode
 *
 * \param[in] isc PORT_ISC_INTDISABLE_gc    = Iterrupt disabled but input buffer enabled
 *                PORT_ISC_BOTHEDGES_gc     = Sense Both Edges
 ...
 *                PORT_ISC_LEVEL_gc         = Sense low Level
 */
static inline void SPI_NSS_set_isc(const PORT_ISC_t isc)
{
	PORTC_pin_set_isc(3, isc);
}

/**
 * \brief Set SPI_NSS inverted mode
 *
 * Enable or disable inverted I/O on a pin
 *
 * \param[in] inverted true  = I/O on SPI_NSS is inverted
 *                     false = I/O on SPI_NSS is not inverted
 */
static inline void SPI_NSS_set_inverted(const bool inverted)
{
	PORTC_pin_set_inverted(3, inverted);
}

/**
 * \brief Set SPI_NSS level
 *
 * Sets output level on a pin
 *
 * \param[in] level true  = Pin level set to "high" state
 *                  false = Pin level set to "low" state
 */
static inline void SPI_NSS_set_level(const bool level)
{
	PORTC_set_pin_level(3, level);
}

/**
 * \brief Toggle output level on SPI_NSS
 *
 * Toggle the pin level
 */
static inline void SPI_NSS_toggle_level()
{
	PORTC_toggle_pin_level(3);
}

/**
 * \brief Get level on SPI_NSS
 *
 * Reads the level on a pin
 */
static inline bool SPI_NSS_get_level()
{
