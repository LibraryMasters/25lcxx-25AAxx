 ...
 * @brief     tick count
 * @return    status code
 *            - system tick in micro seconds
 * @note      none
 */
uint32_t micros(void)
{
//   return u32MicroCounter; 
}

/**
 * @brief     delay function in micro seconds
 * @param[in] usTime is time delay
 * @return    status code
 *            - 0 delay success
 *            - 1 delay failed
 * @note      none
 */
uint8_t delay_us(uint32_t usTime)
{
    volatile uint32_t delayCheck = micros();
    while ((micros() - delayCheck) < usTime);

    if (micros() - delayCheck < usTime) {
    } 
    return 0;
}
